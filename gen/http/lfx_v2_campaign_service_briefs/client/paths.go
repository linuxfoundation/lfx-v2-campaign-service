// Code generated by goa v3.25.3, DO NOT EDIT.
//
// HTTP request path constructors for the lfx-v2-campaign-service-briefs service.
//
// Command:
// $ goa gen github.com/linuxfoundation/lfx-v2-campaign-service/design

package client

import (
	"fmt"
)

// CreateBriefLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service create-brief HTTP endpoint.
func CreateBriefLfxV2CampaignServiceBriefsPath(projectID string) string {
	return fmt.Sprintf("/projects/%v/briefs", projectID)
}

// FindBriefLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service find-brief HTTP endpoint.
func FindBriefLfxV2CampaignServiceBriefsPath(projectID string) string {
	return fmt.Sprintf("/projects/%v/briefs", projectID)
}

// GetBriefLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service get-brief HTTP endpoint.
func GetBriefLfxV2CampaignServiceBriefsPath(projectID string, briefID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v", projectID, briefID)
}

// UpdateBriefLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service update-brief HTTP endpoint.
func UpdateBriefLfxV2CampaignServiceBriefsPath(projectID string, briefID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v", projectID, briefID)
}

// ApproveBriefLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service approve-brief HTTP endpoint.
func ApproveBriefLfxV2CampaignServiceBriefsPath(projectID string, briefID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/approve", projectID, briefID)
}

// DeleteBriefLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service delete-brief HTTP endpoint.
func DeleteBriefLfxV2CampaignServiceBriefsPath(projectID string, briefID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v", projectID, briefID)
}

// FetchEventURLLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service fetch-event-url HTTP endpoint.
func FetchEventURLLfxV2CampaignServiceBriefsPath(projectID string) string {
	return fmt.Sprintf("/projects/%v/fetch-event-url", projectID)
}

// UploadCreativeAssetLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service upload-creative-asset HTTP endpoint.
func UploadCreativeAssetLfxV2CampaignServiceBriefsPath(projectID string, briefID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/creative-assets", projectID, briefID)
}

// CreateCampaignsLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service create-campaigns HTTP endpoint.
func CreateCampaignsLfxV2CampaignServiceBriefsPath(projectID string, briefID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/campaigns", projectID, briefID)
}

// AdoptCampaignLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service adopt-campaign HTTP endpoint.
func AdoptCampaignLfxV2CampaignServiceBriefsPath(projectID string, briefID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/campaigns/adopt", projectID, briefID)
}

// GetCampaignLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service get-campaign HTTP endpoint.
func GetCampaignLfxV2CampaignServiceBriefsPath(projectID string, briefID string, campaignID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/campaigns/%v", projectID, briefID, campaignID)
}

// GetCampaignMetricsLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service get-campaign-metrics HTTP endpoint.
func GetCampaignMetricsLfxV2CampaignServiceBriefsPath(projectID string, briefID string, campaignID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/campaigns/%v/metrics", projectID, briefID, campaignID)
}

// GetCampaignSettingsLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service get-campaign-settings HTTP endpoint.
func GetCampaignSettingsLfxV2CampaignServiceBriefsPath(projectID string, briefID string, campaignID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/campaigns/%v/settings", projectID, briefID, campaignID)
}

// GetBriefMetricsLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service get-brief-metrics HTTP endpoint.
func GetBriefMetricsLfxV2CampaignServiceBriefsPath(projectID string, briefID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/metrics", projectID, briefID)
}

// GenerateEmailCopyLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service generate-email-copy HTTP endpoint.
func GenerateEmailCopyLfxV2CampaignServiceBriefsPath(projectID string, briefID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/email-copy", projectID, briefID)
}

// UpdateCampaignLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service update-campaign HTTP endpoint.
func UpdateCampaignLfxV2CampaignServiceBriefsPath(projectID string, briefID string, campaignID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/campaigns/%v", projectID, briefID, campaignID)
}

// ToggleCampaignStatusLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service toggle-campaign-status HTTP endpoint.
func ToggleCampaignStatusLfxV2CampaignServiceBriefsPath(projectID string, briefID string, campaignID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/campaigns/%v/status", projectID, briefID, campaignID)
}

// ApplyKeywordActionsLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service apply-keyword-actions HTTP endpoint.
func ApplyKeywordActionsLfxV2CampaignServiceBriefsPath(projectID string, briefID string, campaignID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/campaigns/%v/keyword-actions", projectID, briefID, campaignID)
}

// DeleteCampaignLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service delete-campaign HTTP endpoint.
func DeleteCampaignLfxV2CampaignServiceBriefsPath(projectID string, briefID string, campaignID string) string {
	return fmt.Sprintf("/projects/%v/briefs/%v/campaigns/%v", projectID, briefID, campaignID)
}

// GetJobLfxV2CampaignServiceBriefsPath returns the URL path to the lfx-v2-campaign-service-briefs service get-job HTTP endpoint.
func GetJobLfxV2CampaignServiceBriefsPath(projectID string, jobID string) string {
	return fmt.Sprintf("/projects/%v/jobs/%v", projectID, jobID)
}
