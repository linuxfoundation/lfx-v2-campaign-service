...
type Service interface {
	...
	// two projects see the same campaigns when they are configured against the
	// same portal, which is the ordinary case under the LF umbrella, whose
	// foundations share one portal. The portal-wide part is a property of
	// HubSpot's data model rather than a gap in the scoping here, and it is why
	// the create route below needs a warning. The match is HubSpot's own `query`
	// search over its default searchable properties: NOT an exact-name lookup, and
	// NOT relevance-ranked — the CRM v3 search API has no relevance sort, and no
	// `sorts` is sent, so the order is UNSPECIFIED. **Do not read the first row as
	// the best match** — the search is token-based, so a hit can merely share a
	// token with the query. Every match is returned, in the order HubSpot returned
	// them, rather than narrowed to a best one, because choosing between
	...
	// create. **The result set is CAPPED at 200 and there is no paging.** A
	...
	// 200 is HubSpot's own per-request maximum (raised from 100 in September
	// 2024), so the gap between "not in the top N" and "does not exist" is as
	// small as one request can make it. **`capped` reports when that gap is
	// actually open**, derived from HubSpot's own total rather than from the
	// returned count — an exactly-full page and a truncated one are the same
	// length. While `capped` is true the caller must not offer an unqualified
	// create: the campaign it would duplicate may be one of the matches HubSpot
	// did not return. This is NOT a list endpoint under rule 3: it is a keyed
	// query returning the matches for one supplied term. It IS a collection —
	// `campaigns` is an array and `q` narrows it — but a BOUNDED one: a single
	// unpaged request capped at 200, with no cursor and no filters beyond the
	// search term, so rule 3's concern about an open-ended listing surface does
	// not apply.
	...
	// stored connection EXISTS but is not usable as configured. A 401/403 on a
	// connection the PROJECT owns says so in its own words, because retrying
	// another name cannot fix a permission problem. The SAME rejection on the
	// shared LF connection — used when the project has none of its own — is a 500
	// instead: that scope is unaddressable over HTTP, so a 400 would tell the
	// caller to repair a row they cannot reach. Retrying another NAME cannot fix a
	// permission problem. **404 — no HubSpot connection is configured for this
	// project.** Distinct from the 400 above, which means one exists and is
	// broken: the remedy is to connect HubSpot, not to fix a credential. **500 —
	// the shared LF connection was refused on permissions, the stored credential
	// could not be decrypted**, or the service is otherwise faulted BEFORE the
	// request went out. Not the operator's to fix, and not retryable by them. 500
	// is reserved for that pre-send position: a fault discovered AFTER the create
	// returned without error is a 503, because by then the campaign may exist and
	// only this service's reading of the outcome failed. Those three prove nothing
	// reached HubSpot, which is why they are reported as themselves rather than as
	// an unconfirmed outcome: sending an operator to look for a campaign that was
	...
	// connection credential to use, exactly as GET
	// .../connection-google-ads/accounts does, and the read enumerates everything
	// that credential reaches on account_id, not only campaigns this service
	// created. A pure read: nothing is persisted.
