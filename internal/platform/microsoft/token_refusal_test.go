...
// response that is neither a 5xx nor a 429 is not automatically Microsoft evaluating the stored
// credential.
//
...
// error codes all told the operator "microsoft-ads rejected your stored credential" about a
// credential Microsoft never looked at — sending them to replace a working one while the actual
...
func TestTokenRefusalSentinelsRouteToOppositeOutcomes(t *testing.T) {
	...
	request := fmt.Errorf("%w: microsoft-ads token refresh -> %d", ErrTokenRequestRejected, http.StatusNotFound)
...
func TestFetchToken_UnusableBodyKeepsTheStatusVerdict(t *testing.T) {
	...
	_, err := NewClient(testCreds(), testAccount(), WithTokenURL(srv.URL), WithClock(fixedClock())).fetchToken(context.Background())
	if err == nil {
