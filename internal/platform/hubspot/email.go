...
// emailListResponse is the shape of GET /marketing/v3/emails.
type emailListResponse struct {
	Results []Email `json:"results"`
}

// SearchEmails returns marketing emails whose name or subject contains query
// (case-insensitive), most-recently-updated first. Read-only (idempotent).
func (c *Client) SearchEmails(ctx context.Context, query string) ([]Email, error) {
	...
	out := make([]Email, 0, len(resp.Results))
...
func (c *Client) CloneEmail(ctx context.Context, sourceID, cloneName string) (*Email, error) {
	...
	var e Email
	if err := json.Unmarshal(raw, &e); err != nil {
		return nil, fmt.Errorf("hubspot: decode clone response: %w", err)
	}
	if e.ID == "" {
...
func (c *Client) SetSendList(ctx context.Context, id, sendListID string, suppressionListIDs []string, isILS bool) (*Email, error) {
	...
	if isILS {
		...
		// Legacy lists are numeric; drop non-numeric ids defensively.
		include := []any{}
		if n, err := strconv.Atoi(sendListID); err == nil {
			include = append(include, n)
		}
		to["contactLists"] = map[string]any{"include": include, "exclude": suppress}
	}
