...
// ErrCredentialRejected marks Google EVALUATING this connection's stored credential and
// REFUSING it — a revoked or expired refresh token, or application credentials that no longer
// match. It is a fact about the stored credential, decided by Google, and permanent: retrying
...
var ErrCredentialRejected = errors.New("google-ads: the token endpoint refused the stored credential")
...
// the operator Google had rejected their credential and sent them to replace a working one.
var ErrTokenRequestRejected = errors.New("google-ads: the token endpoint refused the request this service built")
