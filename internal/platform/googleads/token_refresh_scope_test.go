...
// The real detached deadline is googleAdsRequestTimeout, which is far longer, so nothing about
...
// socket and a file descriptor alive for the remainder of googleAdsRequestTimeout to produce a
// token no one can read — once per probe, on every connection on the platform, and precisely when
// Google's token endpoint is the thing that is slow, which is when probes are being cancelled.
//
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			why: "the refresh outlived the probe that is its only consumer, holding a goroutine and " +
				"its connection open for the rest of googleAdsRequestTimeout",
		},
		{
