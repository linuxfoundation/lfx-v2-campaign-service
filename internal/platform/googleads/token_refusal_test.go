...
// response that is neither a 5xx nor a 429 is not automatically Google evaluating the stored
// credential.
//
...
// error codes all told the operator "google-ads rejected your stored credential" about a
// credential Google never looked at — sending them to replace a working one while the actual
...
func TestTokenRefusalSentinelsRouteToOppositeOutcomes(t *testing.T) {
	credential := fmt.Errorf("%w: google-ads token refresh -> %d", ErrCredentialRejected, http.StatusBadRequest)
	...
	request := fmt.Errorf("%w: google-ads token refresh -> %d", ErrTokenRequestRejected, http.StatusNotFound)
