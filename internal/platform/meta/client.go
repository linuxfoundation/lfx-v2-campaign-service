...
func (c *Client) uploadImage(ctx context.Context, image []byte, contentType string) (string, error) {
	...
	// The multipart body is FRAMED AROUND the image, never copied around it.
	//
	// The obvious construction writes the image into a bytes.Buffer with multipart.Writer
	// and posts the result. That doubles every image in memory — the caller's slice plus
	// the encoded copy — and holds the copy for the whole retry sequence. On this path
	// that is not a rounding error: the dispatcher already reserves the source slices
	// against a process-wide budget (dispatch.AssetReserver), and the encoded copy sits
	// OUTSIDE it, so five concurrent dispatches added ~150 MiB the aggregate bound did
	// not know about.
	//
	// Multipart framing for a single part is a fixed PREFIX (boundary + headers) and a
	// fixed SUFFIX (closing boundary), both a few hundred bytes and both independent of
	// the payload. So they are generated once, with the image absent, and the request
	// body becomes prefix + image + suffix — the image referenced in place.
	prefix, suffix, contentTypeHeader, ferr := multipartFraming(contentType)
	if ferr != nil {
		return "", ferr
	}
	...
	for attempt := 0; ; attempt++ {
		hash, retryIn, rerr := c.uploadImageAttempt(ctx, path, prefix, image, suffix, contentTypeHeader, attempt)
...
// multipartFraming builds the fixed bytes that surround the image in a single-part
// multipart/form-data body, WITHOUT the image itself.
//
// It returns the prefix (boundary line + part headers), the suffix (closing boundary) and the
// Content-Type header carrying the boundary. Both byte slices are a few hundred bytes and neither
// depends on the payload, which is the property that lets the image be referenced in place rather
// than copied: the request body is prefix + image + suffix.
//
// The framing is generated by mime/multipart itself, with an EMPTY part body, rather than
// hand-assembled. That matters — CRLF placement, the exact header ordering and the closing
// boundary are the encoder's business, and a hand-rolled version would be a second implementation
// of a format this package already depends on being exact. Writing zero bytes to the part yields
// precisely the bytes that would have surrounded a real payload, which is asserted directly by
// TestMultipartFramingMatchesBufferedEncoding.
//
// The part's field name is NOT the contract — Meta's own SDKs send different ones (`source0` in
// Python, `filename` in PHP) against this same endpoint, so the handler accepts any file part.
// Any stable name works; this one is left as-is. The FILENAME is what matters: it makes Graph
// treat the part as a file upload rather than a scalar field, and Meta echoes its basename back
// as the response key. uploadImage reads the single entry by value rather than by that key.
func multipartFraming(contentType string) (prefix, suffix []byte, headerValue string, err error) {
	...
	if _, perr := mw.CreatePart(partHeader); perr != nil {
		return nil, nil, "", fmt.Errorf("build image upload body: %w", perr)
	}
	// Everything written so far precedes the payload. Copied out because Close appends to the
	// same buffer and would otherwise alias this slice.
	prefix = append([]byte(nil), buf.Bytes()...)

	if cerr := mw.Close(); cerr != nil {
		return nil, nil, "", fmt.Errorf("build image upload body: %w", cerr)
	}
	suffix = append([]byte(nil), buf.Bytes()[len(prefix):]...)

	return prefix, suffix, mw.FormDataContentType(), nil
}
...
func (c *Client) uploadImageAttempt(ctx context.Context, path string, prefix, image, suffix []byte, contentTypeHeader string, attempt int) (string, time.Duration, error) {
	const final = time.Duration(-1)

	// A FRESH reader per attempt, over the SAME three slices. This is what preserves
	// replay: an io.Reader is consumed by the send, so reusing one would post an empty
	// body on a retry — a request Meta rejects for a reason unrelated to the throttle that
	// caused the retry. Rebuilding it costs three small headers and copies no payload,
	// because bytes.NewReader is a VIEW over the slice rather than a copy of it.
	//
	// The body is byte-for-byte what a buffered multipart.Writer would have produced; that
	// equivalence is asserted by TestMultipartFramingMatchesBufferedEncoding rather than
	// assumed.
	body := io.MultiReader(bytes.NewReader(prefix), bytes.NewReader(image), bytes.NewReader(suffix))

	req, rerr := http.NewRequestWithContext(ctx, http.MethodPost, c.baseURL+path, body)
	if rerr != nil {
		return "", final, fmt.Errorf("build request: %w", rerr)
	}
	// ContentLength must be set explicitly: net/http can infer it from a *bytes.Reader but
	// not from an io.MultiReader, and without it the request goes out CHUNKED. Meta's
	// /adimages endpoint is not documented to accept a chunked upload, and a wrong or
	// absent length is exactly the class of silent-truncation bug this change must not
	// introduce.
	req.ContentLength = int64(len(prefix)) + int64(len(image)) + int64(len(suffix))
	req.Header.Set("Authorization", "Bearer "+c.creds.AccessToken)
