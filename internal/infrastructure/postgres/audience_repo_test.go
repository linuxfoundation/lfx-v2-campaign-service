...
// TestMigration000019_AddsAudienceUpdatedBy pins the DDL the statements above compile
// against. Only updated_by is added: campaign_audiences has carried created_by since 000005.
func TestMigration000019_AddsAudienceUpdatedBy(t *testing.T) {
	up, err := fs.ReadFile(migrations.FS, "000019_audience_actor_columns.up.sql")
	require.NoError(t, err)
	...
	require.Regexp(t, regexp.MustCompile(`(?i)ADD COLUMN IF NOT EXISTS updated_by JSONB`), upSQL,
		"000019 must add updated_by as JSONB — marshalActor stores the actor as a JSONB "+
	...
	require.NotRegexp(t, regexp.MustCompile(`(?i)UPDATE campaign_audiences SET updated_by`), upSQL,
		"000019 backfills updated_by from existing data. NULL means \"not recorded\"; filling it "+
			"from created_by would assert an edit that never happened.")

	down, err := fs.ReadFile(migrations.FS, "000019_audience_actor_columns.down.sql")
	require.NoError(t, err)
