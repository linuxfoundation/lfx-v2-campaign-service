...
-- not enough, because 000014's IF NOT EXISTS lets a pre-existing index of the same
...
-- re-running 000014's build, and it leaves the old constraint protecting the table
-- meanwhile.
--
...
BEGIN
    ...
    -- not enough: 000014 builds with IF NOT EXISTS, so any pre-existing index that
    -- happens to carry this name makes 000014 a silent no-op AND satisfies a name-only
    ...
    IF NOT EXISTS (
        ...
        RAISE EXCEPTION
            'refusing to drop campaigns_brief_id_platform_key: the replacement index public.uq_campaigns_brief_platform_live is missing, INVALID (a failed CONCURRENTLY build in 000014), or does not match its required definition -- UNIQUE on public.campaigns, keyed on exactly (brief_id, platform), partial WHERE status <> ''deleted''. Dropping the constraint now would leave (brief_id, platform) with no enforceable uniqueness, allowing duplicate paid campaigns. Inspect the index, rebuild it from 000014, then re-run this migration.';
    END IF;
