...
// in CreateCampaigns' Input.Config (delivered here as the Dispatch `config`). Budget is
// in whole units of the ad ACCOUNT's currency (NOT USD — the client does no FX),
// mirroring the meta client.
type googleAdsConfig struct {
	Budget float64 `json:"budget"`
	// Headlines/Descriptions are optional Responsive Search Ad copy overrides (GA-3).
...
//
// GA-3 gave the create path a real ad group + ad, so this now cascades like the reddit
