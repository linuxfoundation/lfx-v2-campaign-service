...
// Creative-asset MIME types. These mirror the Enum on the upload contract
// (design/brief.go) and the CHECK constraint on creative_assets.mime_type: the three
// must move together, since each is the same PNG/JPEG allow-list enforced at a
// different layer (contract decode, handler validation, database).
const (
