...
var AccountMonitorCampaign = Type("account-monitor-campaign", func() {
	...
	Attribute("fetch_failed", Boolean, "True when this row's per-campaign metrics fetch failed upstream. The numeric fields are left at their zero value in that case — a renderer MUST check this before treating zero metrics as a real reading.", func() { Example(false) })
	Attribute("pacing_pct", Float64, "spend / expected-spend * 100. Meaningless when pacing_unknown is true.", func() { Example(87) })
	Attribute("pacing_label", String, "The pacing classification derived from pacing_pct.", func() { Enum("normal", "underspending", "constrained", "overspending") })
