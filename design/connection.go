...
var _ = Service("lfx-v2-campaign-service-connections", func() {
	...
	Method("create-hubspot-campaign", func() {
		Description("Create an LF HubSpot marketing campaign and return the `hs_utm` token HubSpot " +
			...
			"Other failures fall into THREE classes, and the status tells them apart. " +
			"**400 — nothing was created.** Either HubSpot rejected the request on the merits (a " +
			"definite non-429 4xx), or the request was never sent at all (no connection, an inactive " +
			"one, an unusable credential). Both are safe to correct and retry; a 401/403 says so in " +
			"its own words, because retrying another NAME cannot fix a permission problem. " +
			"**404 / 500 — setup and service faults**, reported as themselves for the same reason: " +
			"they prove nothing reached HubSpot, so sending an operator to go look for a campaign " +
			"that was never attempted hides the remedy they actually need. " +
