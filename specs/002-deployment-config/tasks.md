...
- [X] T002 Replace `spec.rules: []` in `charts/lfx-v2-campaign-service/templates/ruleset.yaml` with a complete rule set: a public `allow_all` rule for the `/_campaigns/openapi.*` paths (`oidc` + `anonymous_authenticator`) and one rule per `/campaigns` route group as a **fail-closed placeholder** — `oidc` only (no `anonymous_authenticator`, so a valid token is required) (+ `oidc_contextualizer` when enabled) → unconditional `deny_all` authorizer (rejects all requests; endpoints not built yet) → `create_jwt` finalizer with `aud: {{ .Values.app.audience }}` — with a `TODO(LFXV2-2558)` to re-add `anonymous_authenticator` + replace `deny_all` with `openfga_check` once the campaigns API/FGA model exist. Uses rule-id convention `rule:lfx:lfx-v2-campaign-service:<resource>:<action>`, per `contracts/chart-conformance.md` and research R2 (campaign-service repo)
