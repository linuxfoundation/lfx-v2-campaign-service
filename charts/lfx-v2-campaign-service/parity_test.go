...
// its own schema migrations at boot and migration 000013 is BACKWARD-INCOMPATIBLE: it drops
...
//
// Why this is pinned rather than left to convention: 000012 and 000013 cannot be staged
...
func TestDeploymentUsesRecreateStrategy(t *testing.T) {
	...
	if !strings.Contains(rendered, "type: Recreate") {
		t.Errorf("deployment must set `strategy.type: Recreate`; rendered chart does not.\n"+
			...
			"release run against the post-000013 schema where its bare ON CONFLICT (brief_id, platform) "+
	...
	if strings.Contains(rendered, "RollingUpdate") {
		t.Errorf("deployment must NOT use RollingUpdate while a backward-incompatible migration "+
			"(000013, DROP CONSTRAINT) ships in this release.\nrendered:\n%s", rendered)
	}
}
