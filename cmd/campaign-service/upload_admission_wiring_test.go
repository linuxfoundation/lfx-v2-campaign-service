...
func TestUploadAdmission_IsWiredIntoTheRealChain(t *testing.T) {
	...
	// requests this test parks — it self-adjusts and stays green. That is not hypothetical: the
	// unknown-length price WAS reverted from the floor back to the worst-case ceiling, which
	// moved this test from parking 16 requests to parking 1, and it stayed green throughout.
	// The assertion is that the next one after the last admitted is shed, which holds at either
	// price.
	//
	...
	// TestUploadAdmission_UndeclaredLengthUploadsSerialize in internal/middleware, which
