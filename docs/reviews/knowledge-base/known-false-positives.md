...
## 10. License-header, MegaLinter or `gofmt -s` findings in a tree the repo's own tooling excludes from that check

**The claim:** a file under `gen/`, `specs/` or `.specify/` lacks the license
header, is not `gofmt -s`-clean, or trips a MegaLinter rule.

**Why it is wrong — only for these exact check/path pairs**, each verified
against the configuration at the cited line. Anything outside the table is not
covered by this entry and stays a candidate finding.

| Check | Excluded paths | Proof |
|---|---|---|
| License-header check | `gen/`, `specs/`, `.specify/` | `.github/workflows/license-header-check.yml:18` — `exclude_pattern: "gen,specs,.specify"` |
| MegaLinter (all linters) | `gen/`, `.specify/`, chart `templates/` — **not** `specs/` | `.mega-linter.yml:46` — `FILTER_REGEX_EXCLUDE` names `gen/.*`, `\.specify/.*` and the chart `templates/` YAML; `specs/` is not in it |
| `make check-fmt` (`gofmt -s -l $(GO_FILES)`) | `gen/` | `Makefile:25` — `GO_FILES := $(shell find . -type f -name '*.go' -not -path './gen/*')`; `Makefile:79` |

Boundaries the table implies, stated so they are not inferred the other way:

- **`specs/` is excluded from the license-header check only.** A markdown, YAML
  or other MegaLinter finding under `specs/` is real.
- **`make fmt` does touch `gen/`.** `Makefile:73` runs `go fmt ./...` before the
  `gofmt -s -w $(GO_FILES)` pass at `Makefile:74`, so `gen/` is gofmt-formatted
  (without `-s`) whenever `make fmt` runs; only the `-s` simplification and the
  `check-fmt` gate skip it. `gen/` is regenerated by `make apigen` and never
  hand-edited, which is why the gate leaves it alone.
...
- `specs/` and `.specify/` contain no Go files at the time of writing, so the
  `check-fmt` row does not apply to them either way.

**Provenance:** carried over 2026-09-25 from the retired
`.claude/skills/campaign-service-code-reviewer` skill, which stated the
exclusion more broadly; narrowed the same day to the pairs the configuration
actually proves. No PR thread; the present-day proof is the cited lines.
...
proof** that the rebuttal still holds. The one sanctioned exception is a dated
entry **carried over from the retired repo conventions reviewer** on 2026-09-25
(entries 9 and 10): it has no thread, must say so in its **Provenance** line, and
must cite present-day proof in place of the thread. No further thread-less
entries are added under that exception. A claim that merely went unfixed is not a
