...
`campaigns` needed its own migration because its write path differs in kind. A brief is written on the request goroutine, so the actor is read from the request context at the point of the write. Dispatch is ASYNC: it runs on the orchestrator's root context in a goroutine that outlives the request, so an actor read at the INSERT would be nil for every campaign ever created. `Orchestrator.Start` captures the DECODED actor (never the bearer token, which may have expired by the time the work runs) while the request context is still in hand and threads it down to `ClaimCampaignDispatch`. A campaign row therefore attributes to whoever authorized the spend. The claim INSERT is normally the row's only INSERT — every later write for the same live `(brief, platform)` takes the upsert's conflict arm, which deliberately leaves `created_by` alone and `COALESCE`s `updated_by` so a system re-persist cannot erase the last known human. "Normally", not "always": `upsertCampaignQuery`'s conflict target carries the partial index's `WHERE status <> 'deleted'` predicate, so a soft-deleted row sits outside the index and a later dispatch INSERTs a FRESH campaign rather than resurrecting it. That arm stamps both actor columns from the values passed in, which is why it must keep doing so — the new row is a new campaign with a new authorizer, and treating the insert arm as unreachable would strip the attribution off exactly the campaigns created after a delete.
