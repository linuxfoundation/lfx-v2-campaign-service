...
5. **Attribution.** Inline `created_by` / `updated_by JSONB` columns record the actor on the row itself. `campaign_briefs` carries both as of migration `000015`, `campaigns` as of `000016`, and `connections` always has. `campaign_audiences` carries `created_by` as of `000005` and `updated_by` as of `000019` (LFXV2-3055). It needed the second column because `update-audience` is a published PATCH (`design/audience.go`) and `AudienceRepo.UpdateAudience` performs an in-place `UPDATE`, so an audience CAN be edited and until `000019` its editor was not recorded. Both inserts stamp `updated_by` from the same actor as `created_by` — a row nobody has edited was last touched by whoever created it — and `updateAudienceQuery` is the only statement that changes it afterwards. A NULL means "not recorded" (an unauthenticated write), never "nobody"; existing rows are deliberately not backfilled, since the migration has no evidence that anyone ever edited them. This is not redundant with the Query Service's revision history: campaigns execute under **shared system accounts**, so no ad platform can answer "which person did this", and the index is a derived, rebuildable projection — an audit trail that lives only there is one reindex away from being lost. Revision history (what changed, version over version) remains the Query Service's; who changed it is the row's. **Connections are not indexed** (see [Security](#security)), so their attribution lives **only** in the inline `created_by` / `updated_by JSONB` columns on the connection row — each captures the full actor (`name`, `email`, `username`) so the record stays meaningful even after a user is removed. In all cases the inline actor columns capture the full actor rather than a bare id.

`campaigns` needed its own migration because its write path differs in kind. A brief is written on the request goroutine, so the actor is read from the request context at the point of the write. Dispatch is ASYNC: it runs on the orchestrator's root context in a goroutine that outlives the request, so an actor read at the INSERT would be nil for every campaign ever created. `Orchestrator.Start` captures the DECODED actor (never the bearer token, which may have expired by the time the work runs) while the request context is still in hand and threads it down to `ClaimCampaignDispatch`. A campaign row therefore attributes to whoever authorized the spend. The claim INSERT is normally the row's only INSERT, and the upsert that follows FINALIZES that claim rather than revisiting an earlier dispatch's row: `dispatchPlatform` upserts only when the claim was won, and every `!claimed` branch returns first (reuse, reconcile, skip). The conflict arm deliberately leaves `created_by` alone and `COALESCE`s `updated_by` so a system re-persist cannot erase the last known human — the `created_by` half being a guarantee about the repository method's contract, since no orchestrator path reaches that arm without having just stamped the row itself. A soft delete does not change that. `claimCampaignDispatchQuery` and `upsertCampaignQuery` both carry the partial index's `WHERE status <> 'deleted'` predicate on their conflict target, so a deleted row sits outside the index — and since dispatch always claims before it upserts, the statement that INSERTs the FRESH campaign after a delete, and stamps its `created_by`, is the CLAIM. The upsert then conflicts with the row the claim just wrote. (A `pending` row cannot be soft-deleted at all: `CampaignStatusDeletable` is a whitelist of settled statuses, so only a completed campaign ever reaches the deleted state.) The upsert's INSERT arm still stamps both actor columns, and must keep doing so, but for a narrower case: the claim row being removed between the claim and the upsert — an operator clearing an apparently-stuck claim, or a concurrent `DeleteDispatchClaim` — which is the only path that creates a campaign row with no live claim in front of it.
...
-- gen_random_uuid() is in PostgreSQL core since v13 (no pgcrypto extension required); target: PG 16.
    ...
    created_by   JSONB,                                -- {name,email,username} — inline attribution
                                                       --   (connections are NOT indexed, so these are
                                                       --   the sole attribution source; see Security)
    updated_by   JSONB,                                -- {name,email,username} — last actor to modify
...
CREATE TABLE google_ads_connections (
    -- common columns (see above): id, project_id, label, account_id, credentials,
...
CREATE TABLE campaign_briefs (
    ...
    created_by    JSONB,                               -- actor who created it (migration 000015)
    updated_by    JSONB,                               -- actor who last wrote it (migration 000015)
    created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at    TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE campaigns (
    ...
    ran_on_system_account BOOLEAN,                       -- which ad account served this campaign, at creation (migration 000027).
                                                         -- NULL = provenance not recorded (pre-000027 rows, and any write that
                                                         -- cannot know it — e.g. AdoptCampaign); FALSE = the project's own
...
- Connections are **not** indexed into the Query Service — they are singleton per project with no cross-project listing consumer, so they are read directly via `GET /projects/{projectId}/connection-{provider}`. (Briefs and campaigns *are* indexed for listing/history; connections are not.) Inline `created_by` / `updated_by` are retained on the connection row for attribution.
