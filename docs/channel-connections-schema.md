...
5. **Attribution.** Inline `created_by` / `updated_by JSONB` columns record the actor on the row itself. `campaign_briefs` carries them as of migration `000015`; `connections` and `campaign_audiences` always have. This is not redundant with the Query Service's revision history: campaigns execute under **shared system accounts**, so no ad platform can answer "which person did this", and the index is a derived, rebuildable projection — an audit trail that lives only there is one reindex away from being lost. Revision history (what changed, version over version) remains the Query Service's; who changed it is the row's. **Connections are not indexed** (see [Security](#security)), so their attribution lives **only** in the inline `created_by` / `updated_by JSONB` columns on the connection row — each captures the full actor (`name`, `email`, `username`) so the record stays meaningful even after a user is removed. In all cases the inline actor columns capture the full actor rather than a bare id.
