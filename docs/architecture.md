...
| D4 | **Brief is the funnel unit; campaigns are a collection subordinate to the brief.** `/projects/{p}/briefs/{b}/campaigns/{id}`. Program type is a field on the brief, not a resource. | One brief drives many channel campaigns, each a `campaigns` row with the same `brief_id`, discriminated by `platform`. So the brief is the natural parent; the funnel/program context is a brief attribute. **Shared across the PAID channels, not across delivery surfaces** — migration `000030` made `delivery_type` and `stage` part of a brief's identity, because paid and email are parallel channels on one event and an email campaign is a SERIES with its own brief per stage. So an event has one paid brief driving its paid platforms, plus one brief per email send. |
| D5 | **Query Service owns lists + revision/audit history for briefs and campaigns.** No bespoke list or `*_audit` endpoints/tables. *Who* performed a write is still recorded inline on the row (`created_by` / `updated_by`), which is not a bespoke audit table — see [Persistence](#persistence). Coverage is uneven and worth stating precisely: `campaign_briefs` and `connections` carry BOTH columns; `campaign_audiences` carries `created_by` as of `000005` and `updated_by` as of `000019` (LFXV2-3055) — it needed the second column because `update-audience` is a public PATCH (`design/audience.go`) backed by an in-place `UPDATE` (`AudienceRepo.UpdateAudience`), so until `000019` an audience edit recorded *when* but not *who*. Pre-existing rows are deliberately not backfilled: the migration has no evidence anyone ever edited them, and a NULL means "not recorded", never "nobody"; `campaigns` carries BOTH as of migration `000016`, captured at `Orchestrator.Start` because dispatch is async (see [Persistence](#persistence)). **Connections are not indexed** (singleton per project, no listing/inventory consumer). | The Query Service indexes indexed resources on write and maintains revision history per (re)index; reproducing it here would duplicate platform capability. Connections have no collection to list, so indexing them would add cost with no consumer. |
...
Each resource carries a `version BIGINT` iterator (ETag/If-Match). Connections are **typed per provider** — the diagram shows `google_ads_connections` as the representative; every provider has its own analogous table (see [channel-connections-schema.md](channel-connections-schema.md)). For **briefs and campaigns**, revision history is served by the **Query Service** on each (re)index, so there are no bespoke `*_audit` tables. Attribution is not delegated to it: `campaign_briefs` (migration `000015`) and `campaigns` (migration `000016`) record the acting person inline in `created_by` / `updated_by` — see [Persistence](#persistence). **Connections are not indexed** (singleton per project, no listing/inventory consumer — see [Indexer Resource Type Names](#indexer-resource-type-names)); their `version` still powers ETag/If-Match on the connection `PUT`.
...
erDiagram
    ...
    campaigns {
        ...
        JSONB result
        BIGINT version "ETag/If-Match"
        JSONB created_by "{name,email,username} — captured at Orchestrator.Start (dispatch is async)"
        JSONB updated_by "{name,email,username} — last modifier"
        TIMESTAMPTZ created_at
        TIMESTAMPTZ updated_at
    }
...
The full table definitions — per-provider connection tables **plus** `campaign_briefs` and `campaigns` — live in [channel-connections-schema.md](channel-connections-schema.md) and are not duplicated here. Revision history for briefs and campaigns is served by the Query Service on each (re)index, so this service keeps no separate revision store. Attribution is NOT delegated to it: `campaign_briefs` (migration `000015`) and `campaigns` (migration `000016`) record the acting person inline in `created_by` / `updated_by`, because campaigns execute under shared system accounts — no ad platform can report who acted — and the index is a rebuildable projection that must not be the only home of an audit trail. Connection tables are not indexed (singleton per project; no listing consumer).
