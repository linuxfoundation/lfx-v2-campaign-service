...
2. **Issuing identity** — Heimdall at the gateway authenticates the caller and mints the
   ...
   anything that can reach the pod, and a gateway is a routing decision, not a security
   boundary a service may assume.
...
The full table definitions — per-provider connection tables **plus** `campaign_briefs` and `campaigns` — live in [channel-connections-schema.md](channel-connections-schema.md) and are not duplicated here. Revision history for briefs and campaigns is served by the Query Service on each (re)index, so this service keeps no separate revision store. Attribution is NOT delegated to it: `campaign_briefs` records the acting person inline in `created_by` / `updated_by` (migration `000015`), because campaigns execute under shared system accounts — no ad platform can report who acted — and the index is a rebuildable projection that must not be the only home of an audit trail. Connection tables are not indexed (singleton per project; no listing consumer).
