...
>
> **Installing the system account is out-of-band, by construction.** Because no request can address the reserved scope, the credentials are installed with the `sysacct-bootstrap` binary, which speaks to the repository and encryptor directly: `DATABASE_URL=… CREDENTIAL_ENCRYPTION_KEY=… sysacct-bootstrap -provider google-ads [-account-id …] < creds.json`. It is idempotent (a second run rotates the credential rather than violating the singleton index), reads the credential from stdin rather than a flag, and `-account-id` may be omitted — the account-discovery endpoint above exists to turn a credentials-only system account into one with an account id.
>
