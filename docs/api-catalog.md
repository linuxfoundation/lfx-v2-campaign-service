...
| POST | `/projects/{projectId}/briefs/{brief_id}/creative-assets` | `campaign_manager` | JSON | Upload an image asset for a brief so a Meta ad creative can reference it by id. Synchronous: the image is validated (PNG/JPEG, non-empty, within the size ceiling) and stored in Postgres (`bytea`), returning a `CreativeAsset` (`id`, verified `mime_type`, `byte_size`, SHA-256 `checksum`). The bytes are base64-encoded in the JSON body. Re-uploading identical bytes to the same brief returns the existing asset (idempotent on `(brief_id, checksum)`). Touches **no ad platform** — the account-scoped Meta `image_hash` is resolved later, at campaign dispatch. `400` for an unsupported type, an empty body, or an image that violates the size/dimension limits. |
