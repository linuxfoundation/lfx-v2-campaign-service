...
> ⚠️ **`GET /projects/{projectId}/{provider}/metrics` IS NOT IMPLEMENTED.** It is declared in no `design/` file and exists on no branch; only the two Google Ads rows below and the campaign- and brief-scoped reads under [Optimization](#optimization) are real. The entry is kept as a design sketch, not a contract — do not code against it, and treat its `days` parameter and its `CampaignMonitorResponse` body as unratified (both disagree with every shipped endpoint; see the notes on each). LFXV2-2641 was closed by the commit that shipped `/google-ads/keywords` and `/google-ads/audience`, which stated the ticket's metrics endpoints had already shipped — the ones that had are the brief- and campaign-scoped reads, not this route.
...
> **Umbrella roll-up (TLF across child foundations).** This service never aggregates across projects: `campaign_manager` does not cascade from parent to child (see rule 2), and each project owns only its own connection. A TLF-wide view of, say, all Google Ads spend would be assembled by the **UI backend**, fanning out one per-project read per child foundation the caller has access to and summing the results. Note the per-provider route that sentence assumed is not implemented (see the warning above), so a roll-up built today has to fan out over the brief-scoped read instead. Keeping aggregation out of this service preserves the strict per-project permission boundary and avoids the service resolving project hierarchy.
