...
| POST | `/projects/{projectId}/connection-linkedin-ads/test` | `campaign_manager` | JSON | **LinkedIn is the one provider whose `/test` departs from the row above.** Beyond the shared credential check, it additionally cross-checks the connection's stored `org_id` against LinkedIn's own `reference` field on the configured ad account (`Orchestrator.VerifyAccountOrg` → `LinkedInDispatcher.VerifyAccountOrg` → `linkedin.Client.VerifyAccountOrgReference`), resolved from the project's OWN connection only — never the LF system-account fallback, same trust boundary as the `account-monitor` row below. A confirmed org mismatch, the configured account being absent from a complete ad-account enumeration, or a credential/authorization failure surfaced while walking the enumeration (expired or invalid credentials, an application-authorization rejection, or a 403 from LinkedIn) is folded into an ordinary **FAILED test** (`OK: false`) — each proves the credential cannot perform the verification, so it is reported the same as any other bad connection. A non-authentication failure of the enumeration walk itself (a transport error or pagination fault, not a credential problem) is different: it proves nothing about the account/org agreement, so it is reported as `OK: true` with an advisory message instead of failing the test. Two failure classes escape both of those and return a typed **500** instead: a credential that fails authenticated decryption (`domain.ErrCredentialDecryptionFailed`, service-side, never echoes cipher material into the response) and a defect in this service's own token handling (`domain.ErrServiceDefect`, e.g. a malformed refresh request) — neither is evidence the stored connection is broken, so neither is reported as a failed test. A stored `org_id` that is not a numeric organization id also fails the test (`OK: false`), and is decided without contacting LinkedIn at all: `resolveOrgID` refuses the same value when building the organization URN, so such a connection cannot create a campaign regardless of what the enumeration would have said. `OK: true` on its own does not distinguish a confirmed match from an inconclusive comparison (LinkedIn has no reference on the account to compare against) — see [`internal-service.md`](knowledge/code/internal-service.md) for why that distinction is not this endpoint's to make. |
