...
| Method | Path | FGA relation | Type | Description |
|--------|------|--------------|------|-------------|
| GET | `/projects/{projectId}/audience-builder/capabilities` | `campaign_manager` | JSON | Whether the builder can be used at all, and if not, why. Reports `hubspotConfigured` with a human-readable detail when it is false. **This endpoint returns no error for an unusable connection** — an unusable connection is its ANSWER, which is what lets a client render one explanatory banner with actions disabled instead of nine broken buttons. |
