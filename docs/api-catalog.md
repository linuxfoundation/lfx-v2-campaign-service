...
| GET | `/projects/{projectId}/google-ads/keywords` | `campaign_manager` | JSON | Google Ads keyword performance (top 50 by impressions over `window`, default `last_30_days`). A live read-through: nothing is persisted, and this service stores no keyword — it enumerates nothing this service holds, which is why it is not a list endpoint under rule 3 (same reasoning as `/connection-google-ads/accounts` below). `REMOVED` criteria are excluded, since a removed keyword can never serve and so cannot be acted on. **The rows are capped and `truncated` reports it** — they are the TOP keywords, not the account's full set, so a total over them is not account-wide spend. Each row carries the `criterion_id` + `ad_group_id` pair that `keyword-actions` takes. |
