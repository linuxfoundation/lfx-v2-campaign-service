...
>
> **Installing the system account is out-of-band, by construction.** Because no request can address the reserved scope, the credentials are installed with the service binary's `bootstrap-system-account` subcommand, which speaks to the repository and encryptor directly: `DATABASE_URL=… CREDENTIAL_ENCRYPTION_KEY=… campaign-service bootstrap-system-account -provider google-ads [-account-id …] [-config org_id=…] < creds.json`. Keys use the documented snake_case form. It is idempotent (a second run rotates the credential rather than violating the singleton index), reads the credential from stdin rather than a flag, requires the `-config` keys an adapter refuses to create without (LinkedIn `org_id`, Meta `page_id`, X `funding_instrument_id`), and `-account-id` may be omitted — the account-discovery endpoint above turns a credentials-only system account into one with an account id.
>
