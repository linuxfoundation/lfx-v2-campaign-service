...
| POST | `/projects/{projectId}/audience-builder/preview-count` | `campaign_manager` | JSON | How many DISTINCT people the selection reaches, by unioning list memberships. `list_ids` must be non-empty (`400`). **Above the 25,000-record paging cap, or when the live membership sweep fails, the response reports the SUM of the selected lists' sizes with `exact: false`, never a fabricated exact number** — but that sum is not always an over-count. Above the cap, or when the sweep fails outright, it over-counts by however much the lists overlap (registrant/speaker overlap is the normal case), which is the safe direction. But when HubSpot did not report a size for one of the selected lists at all, the sum omits that list entirely and UNDER-counts instead — the one direction that must never be presented as exact. `reason` always names which direction the estimate's error runs, so a caller never has to guess which caveat applies. |
...
**`compose-master` has TWO distinct `500`s and the status code cannot tell them apart.** Goa maps both the declared `ComposePartial` error and an ordinary internal error to `500`, discriminating with a `goa-error` response header — which a proxying BFF does not see. **The body is therefore the discriminator:** a `ComposePartial` body carries the created suppression list alongside `code` and `message`, while an internal error carries `code` and `message` only.

The distinction is the whole point of declaring the error. `ComposePartial` means **the combined suppression list was created and the master list was not.** Do NOT offer a bare retry on it: the suppression list already exists, and retrying creates a second one. Surface the list that was created, with a link into the portal, and let the operator decide.
