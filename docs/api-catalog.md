...
| Method | Path | FGA relation | Type | Description |
|--------|------|--------------|------|-------------|
| PATCH | `/projects/{projectId}/briefs/{briefId}/campaigns/{id}/status` | `campaign_manager` | JSON | Toggle campaign ACTIVE/PAUSED (Reddit, Meta, LinkedIn, X/Twitter, Google Ads, Microsoft Ads). **409** when the change is refused before the platform is contacted: the campaign is unprovisioned, or the connection row itself is unusable — no stored credential blob, or one too short for the encryptor to authenticate. Those are non-retryable, which is why none of them is a 503. `reason=account_not_selected` (credentials stored, no ad account chosen yet) and the account-mismatch reason (the campaign belongs to a different ad account than the connection now resolves to) are **Google-Ads-only** — Meta's status toggle never checks account selection at all (it targets an existing campaign by platform id and never reads `AccountConfig.AccountID`; see the Platform Connections section). The three credential-state defects — `reason=connection_inactive` (the row's status is not `active`), `reason=credentials_undecodable` (the decrypted blob is not valid JSON) and `reason=credentials_incomplete` (a required credential field is empty), each tagged with `ErrConnectionNotUsable` — are checked and tagged by both the Google Ads and Meta adapters. Google Ads tags a fourth that Meta has no equivalent for: `reason=provider_config_invalid`, the stored `login_customer_id` is not digits-only, so no manager id can be sent. LinkedIn, Reddit, X and Microsoft Ads adapters do not yet tag any of these: their equivalent checks return bare errors that fall through to **503** rather than 409. Tagging those is follow-up work with the adapters. **500** is reserved for a defect nobody but an operator can act on: the project has no connection of its own, fell back to the LF system row, and THAT row is unusable. It is deliberately not a 409 — a 409 tells the caller to repair "this project's connection", which is a scope they do not own and cannot address. The `reason` token is logged, never returned. |
| GET | `/projects/{projectId}/briefs/{briefId}/campaigns/{id}/metrics` | `campaign_manager` | JSON | Read live performance metrics (impressions, clicks, cost, CTR) for one campaign directly from its ad platform. Pure read — never persisted, unlike `GET .../campaigns/{id}`. `window` query param (`today`, `yesterday`, `last_7_days`, `last_14_days`, `last_30_days`, `this_month`, `last_month`; default `last_30_days`, except X Ads which defaults to `last_7_days` since its stats endpoint caps queryable ranges at 7 days) is a closed, platform-agnostic vocabulary — each dispatcher maps it to its own platform's date-range dialect. A platform with no `MetricsReader` wired returns 400. **409** always means "the read was refused before the platform was contacted, and waiting will not change that". Every platform returns it when the campaign is unprovisioned (empty `PlatformCampaignID`), and when the connection row is unusable in one of the two ways the SHARED resolver detects — no stored credential blob (`reason=credentials_absent`) or a blob too short to authenticate (`reason=credential_blob_malformed`). `reason=account_not_selected` (a connection created with credentials only, whose ad account has not been chosen yet) and the account-mismatch reason — the campaign was created under a different ad account than the connection now resolves to; platform campaign ids are account-scoped, so reading one under the wrong account silently yields zeros or another account's numbers, and the fix is to reconnect the original account — are **Google-Ads-only**: Meta's metrics read never checks account selection (it targets an existing campaign by platform id and never reads `AccountConfig.AccountID`, so an account cleared after the campaign was created does not block reading its metrics). Google Ads and Meta both tag the three credential-state defects they check for themselves with `ErrConnectionNotUsable`, so they are 409 too: `reason=connection_inactive` (the row's status is not `active`), `reason=credentials_undecodable` (the decrypted blob is not valid JSON) and `reason=credentials_incomplete` (a required credential field is empty). Google Ads tags a fourth that Meta has no equivalent for: `reason=provider_config_invalid`, the stored `login_customer_id` is not digits-only, so no manager id can be sent. The LinkedIn, Reddit and X adapters emit none of these: they do not verify account identity, and their equivalent non-active/incomplete checks are bare errors that surface as **503**, not 409. Narrowing those to 409 is follow-up work, tracked with the adapters rather than here. **500**, as on the status toggle, is the system-fallback defect: the project has no connection, fell back to the LF system row, and that row is unusable — an operator-only repair on a scope the caller cannot address, so it is not a 409. Support is per-platform (see below). |
