...
>
> **The reserved `system:linuxfoundation` scope is not addressable.** It holds the LF-owned credentials a project falls back to when it has connected no account of its own, and every one of the six endpoints refuses it: `POST` with `400` (the colon cannot satisfy the slug pattern above) and the other five with `404`. `404` rather than `403` — the reserved scope is not a project this API exposes, and answering "forbidden" would confirm to an unauthorized caller that something is there. Dispatch and account discovery reach it internally; nothing reaches it over HTTP.
>
