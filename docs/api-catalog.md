...
| GET | `/projects/{projectId}/briefs/{briefId}/campaigns/{id}/metrics` | `campaign_manager` | JSON | Read live performance metrics (impressions, clicks, cost, CTR) for one campaign directly from the channel that runs it — an ad platform, or HubSpot for the email channel. Pure read — never persisted, unlike `GET .../campaigns/{id}`. `window` query param (`today`, `yesterday`, `last_7_days`, `last_14_days`, `last_30_days`, `this_month`, `last_month`; default `last_30_days`, except X Ads which defaults to `last_7_days` since its stats endpoint caps queryable ranges at 7 days) is a closed, platform-agnostic vocabulary — each dispatcher maps it to its own platform's date-range dialect. A platform with no `MetricsReader` wired returns 400. **409 covers two different moments, and the distinction is operational.** The first is PRE-CONTACT: the read was refused before the channel was called, and waiting will not change that. Every platform returns it when the campaign is unprovisioned (empty `PlatformCampaignID`), and when the connection row is unusable in one of the two ways the SHARED resolver detects — no stored credential blob (`reason=credentials_absent`) or a blob too short to authenticate (`reason=credential_blob_malformed`). **Of the rest, Google Ads and HubSpot verify tenant identity, and the three tagged credential defects are theirs too.** Google Ads also returns 409 for `reason=account_not_selected` (a connection created with credentials only, whose ad account has not been chosen yet) and for an account mismatch — the campaign was created under a different ad account than the connection now resolves to; platform campaign ids are account-scoped, so reading one under the wrong account silently yields zeros or another account's numbers, and the fix is to reconnect the original account. Google Ads and HubSpot each further tag the three defects they check for themselves with `ErrConnectionNotUsable`, so those are 409 too: `reason=connection_inactive` (the row's status is not `active`), `reason=credentials_undecodable` (the decrypted blob is not valid JSON) and `reason=credentials_incomplete` (a required credential field is empty). HubSpot emits no `account_not_selected` 409 — there is no ad account in an email connection to choose — but it DOES emit an account mismatch, against the HubSpot **portal** rather than an ad account. A HubSpot email id is a bare numeric unique only within its portal, so the dispatcher records the portal the private-app token authenticates against (read from `/account-info/v3/details`, not from the optional operator-supplied `portal_id` config, which a credential swap leaves untouched) at create time and refuses the read when the token now reaches a different portal — or when the campaign row records no portal at all, which is every campaign staged before this landed. **The two remedies differ, and the response message is what carries the distinction:** a recorded-but-different portal is `ErrCampaignAccountMismatch` and can be repaired by reconnecting the original portal; a row with no recorded portal at all is the narrower `ErrCampaignProvenanceUnknown` — there is nothing to reconnect to, so its message says to re-dispatch instead. The refusal is deliberate rather than a best guess, because reading across a re-point is wrong in both directions — a same-numeric collision reports another portal's opens and clicks as this campaign's, and no collision reports "not sent yet" for an email that was sent. The second moment is POST-CONTACT and belongs to HubSpot alone: `GetEmailMetrics` succeeds and matches no sent email in the window, which `internal/dispatch/hubspot.go` tags with `domain.ErrNoMetricsInWindow` so it lands on 409 rather than the 503 default. Nothing is broken — a staged draft nobody has sent yet is the ordinary state of this channel between `Dispatch` and the send — so read this 409 as "no data", not as "repair your connection". The response body does not separate the two moments (`ConflictError` carries only `code` and `message`), so the message text is what distinguishes them: the pre-contact ones name the connection or the provisioning state, this one names the window. The Meta, LinkedIn, Reddit and X adapters emit none of these: they do not verify account identity, and their equivalent non-active/incomplete checks are bare errors that surface as **503**, not 409. Narrowing those is follow-up work, tracked with the adapters rather than here. **500**, as on the status toggle, is the system-fallback defect: the project has no connection, fell back to the LF system row, and that row is unusable — an operator-only repair on a scope the caller cannot address, so it is not a 409. Support is per-platform (see below). |
