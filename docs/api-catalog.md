...
| POST | `/projects/{projectId}/connection-linkedin-ads/test` | `campaign_manager` | JSON | **LinkedIn is the one provider whose `/test` departs from the row above.** Beyond the shared credential check, it additionally cross-checks the connection's stored `org_id` against LinkedIn's own `reference` field on the configured ad account (`Orchestrator.VerifyAccountOrg` → `LinkedInDispatcher.VerifyAccountOrg` → `linkedin.Client.VerifyAccountOrgReference`), resolved from the project's OWN connection only — never the LF system-account fallback, same trust boundary as the `account-monitor` row below. A confirmed org mismatch, the configured account being absent from a complete ad-account enumeration, or any failure resolving the connection is folded into an ordinary **FAILED test** (`OK: false`), not a 5xx — a service-level **503** is reserved for the orchestrator itself being unwired, which cannot happen in a normal deployment. A failure of the enumeration walk itself (not the pairing) is different: it proves nothing about the account/org agreement, so it is reported as `OK: true` with an advisory message instead of failing the test. `OK: true` on its own does not distinguish a confirmed match from an inconclusive comparison (no reference to compare, a malformed configured org id) — see [`internal-service.md`](knowledge/code/internal-service.md) for why that distinction is not this endpoint's to make. |
