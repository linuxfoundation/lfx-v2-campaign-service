...
`uq_campaign_briefs_project_event_delivery_stage` (000030, replacing 000003's narrower
`uq_campaign_briefs_project_event`) keeps one live brief per
(project, event slug, delivery type, stage) — an event carries a paid brief and an email
series at once, so the slug alone does not identify one.
...
`uq_campaign_briefs_project_event_delivery_stage`, which breaks `go test -count=2` and, worse, turns a
