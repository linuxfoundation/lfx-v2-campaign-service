...
description: "How the short pre-PR review block in CLAUDE.md points at the central /lfx-skills:lfx-pre-pr-review skill and names the two repo-owned values it reads: the knowledge-base review skill and the deterministic preflight."
resource: "CLAUDE.md"
---
...
The block names the skill, restates the two rules that must not drift out of
mind (one fix commit for the whole round; no local reviews once the PR is open),
and carries the two values the skill reads from the repo. This concept explains
that shape; it does not restate the procedure — read the skill for that.

## The two values

| Value in the block | This repo's setting | What it is |
|---|---|---|
| KB review skill | `/campaign-service-learnings-reviewer` | the repo-owned knowledge-base reviewer, launched by the central skill as the knowledge-base role of the round |
| Preflight | `make check-fmt && make lint && make test && go run ./cmd/okfvalidate ./docs/knowledge` | the deterministic, non-fixing checks CI also runs |

These two lines are the only facts about the lifecycle that belong to this
repo. Everything else — how the round is launched, how the range is pinned,
what a failed reviewer means, when the fix commit happens — is the central
skill's business, and a fix to it is one change there rather than one per repo.
...
reviews the whole branch measured from `origin/main`, a waiver added anywhere
on the branch can never suppress a finding about that branch.
...
report, as the central skill directs. When the fix commit changes what the
code does, the knowledge-bundle upkeep rules in `CLAUDE.md` apply to it.
