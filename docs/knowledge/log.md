...
**Update** — Fixed the group-orphan persistence gap (orchestrator `dispatchPlatform`): a retained partial with an EMPTY `PlatformCampaignID` but a non-empty `Result` (LinkedIn's group-created-but-campaign-failed case) was DROPPED — persist only fired for a non-empty upstream id — leaving the orphaned group id + status unrecorded while the pending claim blocked the pair with no trace. Now persists whenever `campaign != nil && (PlatformCampaignID != "" || len(Result) > 0)`, keeping the row `pending` with the empty id (which keeps it OUT of the idempotency fast-path so it can't false-succeed) and the `Result` blob (so the orphan is reconcilable). The empty-id row is NOT auto-re-attempted — a later job's `ClaimCampaignDispatch` conflicts with the retained pending row (`ON CONFLICT DO NOTHING`) and skips dispatch, so the pair stays blocked pending reconciliation / resume support (LFXV2-2665). This PR only makes the orphan RECORDED, not auto-resumed. Added `TestOrchestrator_GroupOrphanPartialIsPersisted` (+ a negative case: an empty-id, empty-`Result` partial under error is NOT persisted).
